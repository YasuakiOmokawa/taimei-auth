...
// SessionGuard は初回 mount のみ session を再評価するため navigate ではなく full reload で断つ
export type AuthChange = "signOut" | "deleteAccount";

// cookieCache (最大 5 分) は session 失効に追随せず、entry redirect 経由だと削除済み user が事業所作成へ流れる
...
// redirectToSignIn だと auth-entry-redirect が同じ session で事業所登録へ送り返しループする
